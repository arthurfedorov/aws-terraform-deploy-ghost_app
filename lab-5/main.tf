...
# Create ec2 target group for load balancer
...
# Create fargate target group for load balancer
...
resource "null_resource" "docker_push" {
  provisioner "local-exec" {
    command = templatefile("./docker_login.sh.tpl", {
      aws_account_id          = "${data.aws_caller_identity.current.account_id}"
      aws_region              = "${data.aws_region.current.name}"
      ghost_app_image_version = "${var.ghost_app_image_version}"
      ghost_app_image         = "${var.ghost_app_image}"
    })
  }
}
...
resource "aws_ecs_task_definition" "task_def_ghost" {
  ...
  container_definitions = templatefile("./container-definition.json.tpl", {
    DB_NAME                 = "${aws_db_instance.ghost.db_name}"
    DB_USER                 = "${aws_db_instance.ghost.username}"
    DB_URL                  = "${aws_db_instance.ghost.address}"
